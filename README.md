...
검색 품질(독립 라벨 104건, 정답 = 청크 1개): 벡터 검색 Recall@40 90.4%, 리랭킹 후 최종 top-5 적중 71.2%. 이 값은 하한선이다 — 라벨 감사(2026-09-26)에서 미스의 상당수가 라벨 오류로 확인됐다. 출처를 가려 다중 정답으로 다시 판정한 결과(Claude 1인 판정) 리랭크 top-5 적중은 엄격 92.3%·관대 98.1%, vector-only는 84.6%·96.2%다([docs/05 §5.5](docs/05-evaluation.md#55-검색-품질-오프라인-지표)).
