...
**후보를 40개나 뽑는 이유.** 독립 라벨 104건 기준 벡터 검색 Recall@5는 59.6%인데 Recall@40은 90.4%였다
([05-evaluation.md](05-evaluation.md#55-검색-품질-오프라인-지표)). 1차 검색은
...
줄어든다. 다만 완벽하지는 않다 — 독립 라벨 104건 기준 최종 top-5 히트율은 71.2%로, 정답이 후보
안에 있어도(벡터검색 Recall@35 87.5%) 리랭킹 결과에 그 라벨 청크가 없는 경우가 16%p 정도 있다. 다만 이
숫자는 단일 청크 정답 라벨 기준의 하한선이다 — 2026-09-26 라벨 감사에서 그 미스의 상당수가 라벨 오류였고, 리랭킹
모델을 gpt-4o로 바꿔도 개선이 없었다. 출처를 가려 다중 정답으로 다시 판정하면(Claude 1인 판정) 리랭크 top-5 적중이
엄격 92.3%, vector-only 84.6%로 리랭킹 이득(+8문항)은 유지된다(자세한 수치는
[05-evaluation.md](05-evaluation.md#55-검색-품질-오프라인-지표) 참고).

**압축 폭 35, 최종 개수 5는 이 104건으로 확인했다(최적화가 아니라 평평한 구간 확인).** 압축 폭을 40(=벡터검색이 가져온 걸 안
자르는 것)으로 늘려도 히트율이 71.2%로 완전히 동일해서 35를 유지했고, 최종 개수를 10으로 늘려도
+3.8%p뿐이라(3→5의 +13.5%p 대비 수확체감) 5를 유지했다.
...
| 항목 | 값 |
|---|---|
| 그림이 붙은 답변 | 122 / 300 (41%) — 2026-09-21~22 baseline 기준, 최신 baseline 재계산 전 |
