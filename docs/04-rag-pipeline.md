...
전체 흐름은 [03-architecture.md](03-architecture.md#한-턴의-흐름)에 있다. 이 문서는 각 단계에서
무엇을 선택했고 **왜 그렇게 했는지**를 다룬다.
...
```
질문에 에러코드가 있는가?  ──예──► error_codes 정확 매칭 (LLM 0회, 0ms)
        │아니오
기능 질문이고 표에 '없음'?  ──예──► "이 모델에는 없습니다" 확정
        │아니오
        └──────────────────────────► 벡터 검색 + 리랭킹
```
