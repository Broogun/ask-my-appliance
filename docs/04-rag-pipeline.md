...
**후보를 40개나 뽑는 이유.** 질문 104건 기준(엄격) 벡터 검색 Recall@5는 84.6%인데 리랭킹에 넘기는 후보 35개 안에는 93.3%(Recall@35)가 들어 있다
...
줄어든다. 질문 104건을 출처를 가려 여러 청크를 정답으로 허용해 판정한 기준(엄격)으로, 최종 top-5 적중은 벡터만 쓰면 84.6%,
리랭킹 후 92.3%다. 벡터 검색이 놓친 8문항을 리랭킹이 모두 되찾았고 반대로 잃은 문항은 없다(자세한 수치는
[05-evaluation.md](05-evaluation.md#55-검색-품질-오프라인-지표) 참고).

**압축 폭 35, 최종 개수 5는 같은 104건으로 확인했다(최적화가 아니라 평평한 구간 확인).** 압축 폭을 15·20·40으로 바꿔도 적중이
95~96/104로 구분되지 않아 35를 유지했고, 최종 개수는 1→3이 +10문항인 반면 3→5는 +2문항, 5→10은 +1문항이라 수확체감이 뚜렷하다.
검색 적중만 보면 3도 5에 근접하지만, 근거가 여러 청크에 나뉜 답변·안전 경고 등을 위한 여유로 5를 유지했다(최종 답변 품질은 미측정).
