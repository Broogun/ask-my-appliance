...
줄어든다. 다만 완벽하지는 않다 — 독립 라벨 99건 기준 최종 top-5 히트율은 65.7%로, 정답이 후보
안에 있어도(벡터검색 Recall@35 88.9%) 리랭킹이 놓치는 비율이 23%p 정도 있다(자세한 수치는
[05-evaluation.md](05-evaluation.md#55-검색-품질-오프라인-지표) 참고) — 이 리랭킹 정확도 자체를
더 개선하는 건 향후 과제로 남아 있다.

**압축 폭 35, 최종 개수 5는 이 99건으로 직접 검증했다.** 압축 폭을 40(=벡터검색이 가져온 걸 안
자르는 것)으로 늘려도 히트율이 65.7%로 완전히 동일해서 35를 유지했고, 최종 개수를 10으로 늘려도
+3.1%p뿐이라(3→5의 +9.1%p 대비 수확체감) 5를 유지했다.
