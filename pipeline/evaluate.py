...
# 2026-09 확장: LG/삼성 x 에어컨/냉장고/세탁기 6개 조합을 골고루 커버하고,
# 에러코드 조회·증상 기반·설정/조작·브랜드 특화 기능·복합 질문·모호한(브랜드
# 미언급) 질문·안전 관련 질문까지 다양한 케이스를 넣었다. 에러코드는 실제
# data/{lg,samsung}_*_errors.json에 있는 코드 기준으로 골라서 keyword_hit이
# 실제 문서 내용과 맞물리게 했다.
COMMON_QUESTIONS = [
    # 기존 7개 (그대로 유지 - 과거 실험과의 비교 가능성)
    ("에어컨 UE 오류가 뭐야?", "UE", "에러코드 단순 조회 (LG 세탁기, 원문 그대로 유지)"),
    ...
    # LG 에어컨
    ("에어컨에서 CH04 에러가 떴어", "CH04", "에러코드 단순 조회 (LG 에어컨)"),
    ("LG 에어컨 실내기랑 실외기 통신 에러 나요", "통신", "에러코드 단순 조회 (LG 에어컨, CH05계열)"),
    ("에어컨 실외기에서 이상한 소리가 나요", "실외기", "증상 기반 (LG 에어컨)"),

    # LG 냉장고
    ("냉장고에 성에가 계속 생겨요", "제상", "증상 기반 (LG 냉장고)"),
    ("냉장고 Er FF 에러 뜨는데 뭐야", "FF", "에러코드 단순 조회 (LG 냉장고)"),

    # LG 세탁기
    ("세탁기 도어가 안 열려요", "도어", "증상 기반 (LG 세탁기)"),

    # 삼성 에어컨
    ("삼성 에어컨 C101 에러 나요", "C101", "에러코드 단순 조회 (삼성 에어컨)"),
    ("에어컨에서 냉매 새는 냄새가 나는 것 같아요", "냉매", "안전 관련 (삼성 에어컨)"),

    # 삼성 냉장고
    ("냉장고에서 딩동딩동 소리가 계속 나요", "도어", "증상 기반 (삼성 냉장고)"),
    ("삼성 비스포크 냉장고 온도 조절 어떻게 해?", "온도", "브랜드 특화 기능 (삼성 냉장고)"),

    # 삼성 세탁기
    ("삼성 세탁기 UE 에러 떴어요", "UE", "에러코드 단순 조회 (삼성 세탁기)"),
    ("세탁기에서 배수가 안 돼요", "배수", "증상 기반 (삼성 세탁기)"),

    # 브랜드/모델 미언급 - 모호한 질문
    ("에어컨에서 타는 냄새 나는데 계속 써도 돼?", "전원", "안전 관련 (브랜드 미언급)"),
    ("냉장고에서 이상한 소리 나는데 고장인가요?", "소리", "모호한 질문 (브랜드 미언급)"),
    ("냉장고 문이 잘 안 닫히는데 어떻게 고쳐?", "문", "증상 기반 (브랜드 미언급)"),
]
