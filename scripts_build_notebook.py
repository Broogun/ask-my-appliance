# -*- coding: utf-8 -*-
"""PDF 청킹 + RDB/벡터DB 라우팅 질의응답 과정을 정리한 ipynb를 생성한다."""
import json

md = lambda src: {"cell_type": "markdown", "metadata": {}, "source": src.splitlines(keepends=True)}
...
cells.append(md("## 1. OCR이 필요한 문서인가? — 진단 결과"))
...
cells.append(md("## 2. 청킹 방식 비교: 폰트 크기 추정 vs PDF 내장 목차(TOC)"))
...
cells.append(md("## 3. 청크를 문서({id, text, metadata})로 변환"))
...
docs[10]""",
    outputs=stream_output("최종 문서(청크) 수: 77개  (19개 섹션을 700자 단위로 추가 분할)\n") + [{
        "output_type": "execute_result",
        "execution_count": 3,
        "data": {"text/plain": [
...
cells.append(md("## 5. 질의응답 라우터 동작 확인 — 실제 OpenAI(gpt-4o-mini) 결과"))
...
cells.append(md("이전(로컬 3B, 벡터 검색만) 응답은 \"CH04와 FL은 다른 에러코드\"라고 잘못 답했었다. RDB 경로 + OpenAI로 바꾸니 정답이 나온다."))
...
cells.append(md("이전엔 \"찾을 수 없음\"으로 검색 자체가 누락됐던 케이스인데, RDB 별칭 조회로 정확히 찾아낸다."))
...
with open("notebooks/pdf_chunking_and_qa.ipynb", "w", encoding="utf-8") as f:
    json.dump(notebook, f, ensure_ascii=False, indent=1)

print("저장 완료: notebooks/pdf_chunking_and_qa.ipynb")
