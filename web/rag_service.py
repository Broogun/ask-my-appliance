...
5. 형식: 첫 줄에 결론 한 문장 → 확인·조치를 설명서 순서대로 번호 목록. 증상·고장 질문이면 마지막에 "그래도 해결되지 않으면
   ...
   있으면(예: 근거 안에 "확인 A를 했나요?"/"확인 B를 했나요?"처럼 별개의 하위 증상이 나열돼 있으면) 그걸 하나로
   뭉뚱그리지 말고 각각 번호를 따로 매겨서, 항목마다 "왜 그런지" 이유를 한 줄 덧붙이세요. 아래는 형식만 보여주는
   자리표시자 예시입니다(실제 가전·증상 내용이 아니므로 이 문구를 그대로 답변에 쓰지 말고, 반드시 실제
   [근거]의 내용으로 채우세요 - 대괄호([증상], [공통 원인] 등)는 표시 문법일 뿐이니 답변에 대괄호를
   그대로 남기지 말고 실제 문장으로 바꿔 쓰세요):
   나쁜 예(항목을 하나로 뭉침):
     "[증상]의 원인은 [공통 원인]입니다.
     1. [조치 A].
     2. [조치 B]."
   좋은 예(근거에 있는 항목 수만큼, 이유 포함):
     "[증상]은 대부분 [공통 원인] 때문입니다.
     1. [확인 사항 A]인가요? [그 이유를 설명]. [해당 조치].
     2. [확인 사항 B]인가요? [그 이유를 설명]. [해당 조치].
     3. [확인 사항 C]인가요? [그 이유를 설명]. [해당 조치]."
...
def build_web_context(res: RetrievalResult) -> str:
    ...
    for i, (c, _) in enumerate(res.used, 1):
        ...
        # 모델별로 미리 확인해 둔 고장 진단/패널 설정 챕터(rag_myretriever._structural)는
        # 제목이 "고장신고 전 확인하기"처럼 뭉뚱그려진 모델이 많아, 규칙 2의 제목-질문
        # 비교만으로는 LLM이 안 맞는다고 오판해 통째로 거절하는 걸 실측 확인함
        # (2026-09-22). 이 근거는 이미 질문 유형에 맞다고 코드가 확인한 것이므로,
        # 제목 매칭에 맡기지 않고 그 사실을 명시해서 규칙 2를 이 근거에 한해 무력화한다.
        struct_mark = "[✔ 이 모델의 고장/에러 확인 절차로 사전 확인됨 — 제목이 안 맞아 보여도 이 근거로 답하세요] " if c.get("is_structural") else ""
        parts.append(f"[근거 {i}] ({src}) {tag_mark}{struct_mark}{title}\n{body}")
    ctx = head + "\n\n" + "\n\n".join(parts)
    if any(c.get("is_structural") for c, _ in res.used):
        # 위 표시만으로는 부족했다 - "점검문자"/"코드" 표만 있고 "화면에 표시됩니다"처럼
        # 명시적으로 이어주는 문장이 청킹 경계에서 잘린 모델(삼성 스타일러 등)은, 표
        # 자체가 에러 코드 확인 방법이라는 걸 LLM이 규칙 1(설명서 밖 내용 금지) 때문에
        # 스스로 연결 짓지 못하고 여전히 거절하는 걸 실측 확인함(2026-09-22, DF60R8300WG/
        # DF90H24R5C - 표 제목이 "고장신고 전 확인하기"만 있고 "코드"/"점검문자" 열이
        # 있는데도 거절). 그 해석 규칙 자체를 코드로 명시해서 준다.
        ctx += ("\n\n[안내] 위 근거 중 '점검문자'/'정보 코드'/'코드' 항목이 있는 표는 그 자체가 "
                "이 제품이 고장 시 화면(디스플레이)에 표시하는 에러 코드/메시지 목록입니다. "
                "'화면에 표시됩니다'라는 문장이 근거에 따로 없어도, 이 표가 에러 코드 확인 "
                "방법에 대한 답이라고 보고 표에 있는 코드 이름들을 그대로 안내하세요.")
    return ctx
